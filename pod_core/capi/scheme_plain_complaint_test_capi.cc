...
class WrapperSession {
 public:
  WrapperSession(handle_t c_a, uint8_t const* c_self_id,
    ...
    h_ = E_PlainComplaintSessionNew(c_a, c_self_id, c_peer_id);
    if (!h_) throw std::runtime_error("");
  }
  ~WrapperSession() {
    if (!E_PlainComplaintSessionFree(h_)) abort();
  }
  handle_t h() const { return h_; }
...
class WrapperClient {
 public:
  WrapperClient(handle_t c_b, uint8_t const* c_self_id,
    ...
    h_ = E_PlainComplaintClientNew(c_b, c_self_id, c_peer_id, c_demand,
                                   c_demand_count);
    if (!h_) throw std::runtime_error("");
  }
  ~WrapperClient() {
    if (!E_PlainComplaintClientFree(h_)) abort();
  }
  handle_t h() const { return h_; }
...
bool Test(std::string const& output_path, WrapperAliceData const& a,
  ...
  if (evil) E_PlainComplaintSessionSetEvil(session.h());
  ...
  if (!E_PlainComplaintClientGetRequest(client.h(), request_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_PlainComplaintSessionOnRequest(session.h(), request_file.c_str(),
                                        response_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_PlainComplaintClientOnResponse(client.h(), response_file.c_str(),
                                        receipt_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_PlainComplaintSessionOnReceipt(session.h(), receipt_file.c_str(),
                                        secret_file.c_str())) {
    assert(false);
    return false;
  }
  ...
  if (!evil) {
    if (!E_PlainComplaintClientOnSecret(client.h(), secret_file.c_str())) {
      assert(false);
      return false;
    }

    if (!E_PlainComplaintClientSaveDecrypted(client.h(), output_file.c_str())) {
      assert(false);
      return false;
    }
  } else {
    ...
    if (!E_PlainComplaintClientGenerateClaim(client.h(), claim_file.c_str())) {
      assert(false);
      return false;
    }
