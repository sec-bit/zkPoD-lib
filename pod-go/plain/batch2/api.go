...
// NewSellerSession provides the Go interface for E_PlainBatch2SessionNew().
func NewSellerSession(
	...
	if session == nil {
		...
		return nil, fmt.Errorf(
			"E_PlainBatch2SessionNew(%v, %v, %v) failed",
...
// Free provides the Go interface for E_PlainBatch2SessionFree()
func (session *SellerSession) Free() error {
	handle := C.handle_t(session.handle)
	ret := bool(C.E_PlainBatch2SessionFree(handle))
	if !ret {
		return fmt.Errorf("E_PlainBatch2SessionFree(%v) failed", handle)
	}
	return session.a.Free()
}

// OnRequest provides the Go interface for E_PlainBatch2SessionOnRequest().
func (session *SellerSession) OnRequest(requestFile, responseFile string) error {
	...
	ret := bool(C.E_PlainBatch2SessionOnRequest(
		handle, requestFileCStr, responseFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_PlainBatch2SessionOnRequest(%v, %s, %s) failed",
...
// OnReceipt provides the Go interface for E_PlainBatch2SessionOnReceipt()
func (session *SellerSession) OnReceipt(receiptFile, secretFile string) error {
	...
	ret := bool(C.E_PlainBatch2SessionOnReceipt(
		handle, receiptFileCStr, secretFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_PlainBatch2SessionOnReceipt(%v, %s, %s) failed",
...
// NewBuyerSession provides the Go interface for E_PlainBatch2ClientNew()
func NewBuyerSession(
	...
	if session == nil {
		b.Free()
		return nil, fmt.Errorf(
			"E_PlainBatch2ClientNew(%v, %v, %v, %v, %d) failed",
...
// Free provides the Go interface for E_PlainBatch2ClientFree()
func (session *BuyerSession) Free() error {
	handle := C.handle_t(session.handle)
	ret := bool(C.E_PlainBatch2ClientFree(handle))
	if !ret {
		return fmt.Errorf("E_PlainBatch2ClientFree(%v) failed", handle)
	}
	return session.b.Free()
}

// GetRequest provides the Go interface for E_PlainBatch2ClientGetRequest()
func (session *BuyerSession) GetRequest(requestFile string) error {
	...
	if ret := bool(
		C.E_PlainBatch2ClientGetRequest(handle, requestFileCStr)); !ret {
		return fmt.Errorf("E_PlainBatch2ClientGetRequest(%v, %s) failed",
			handle, requestFile)
	}
...
// OnResponse provides the Go interface for E_PlainBatch2ClientOnResponse()
func (session *BuyerSession) OnResponse(responseFile, receiptFile string) error {
	...
	ret := bool(C.E_PlainBatch2ClientOnResponse(
		handle, responseFileCStr, receiptFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_PlainBatch2ClientOnResponse(%v, %s, %s) failed",
...
// OnSecret provides the Go interface for E_PlainBatch2ClientOnSecret()
func (session *BuyerSession) OnSecret(secretFile string) error {
	...
	ret := bool(C.E_PlainBatch2ClientOnSecret(handle, secretFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_PlainBatch2ClientOnSecret(%v, %s) failed",
			handle, secretFile)
	}
...
// Decrypt provides the Go interface for E_PlainBatch2ClientSaveDecrypted()
func (session *BuyerSession) Decrypt(outFile string) error {
	...
	ret := bool(C.E_PlainBatch2ClientSaveDecrypted(
		handle, outFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_PlainBatch2ClientSaveDecrypted(%v, %s) failed",
			handle, outFile)
	}
