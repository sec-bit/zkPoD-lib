...
// NewSellerSession provides the Go interface for E_PlainOtBatch3SessionNew().
func NewSellerSession(
	...
	if session == nil {
		...
		return nil, fmt.Errorf(
			"E_PlainOtBatch3SessionNew(%v, %v, %v) failed",
...
// Free provides the Go interface for E_PlainOtBatch3SessionFree()
func (session *SellerSession) Free() error {
	handle := C.handle_t(session.handle)
	ret := bool(C.E_PlainOtBatch3SessionFree(handle))
	if !ret {
		return fmt.Errorf("E_PlainOtBatch3SessionFree(%v) failed", handle)
	}
	return session.a.Free()
}

// GetNegoRequest provides the Go interface for E_PlainOtBatch3SessionGetNegoRequest()
func (session *SellerSession) GetNegoRequest(requestFile string) error {
	...
	if ret := bool(
		C.E_PlainOtBatch3SessionGetNegoRequest(handle, requestFileCStr)); !ret {
		return fmt.Errorf("E_PlainOtBatch3SessionGetNegoRequest(%v, %s) failed",
			handle, requestFile)
	}
...
// OnNegoRequest provides the Go interface for E_PlainOtBatch3SessionOnNegoRequest()
func (session *SellerSession) OnNegoRequest(requestFile, responseFile string) error {
	...
	if ret := bool(C.E_PlainOtBatch3SessionOnNegoRequest(
		handle, requestFileCStr, responseFileCStr)); !ret {
		return fmt.Errorf(
			"E_PlainOtBatch3SessionOnNegoRequest(%v, %s, %s) failed",
...
// OnNegoResponse provides the Go interface for E_PlainOtBatch3SessionOnNegoResponse()
func (session *SellerSession) OnNegoResponse(responseFile string) error {
	...
	if ret := bool(C.E_PlainOtBatch3SessionOnNegoResponse(
		handle, responseFileCStr)); !ret {
		return fmt.Errorf(
			"E_PlainOtBatch3SessionOnNegoResponse(%v, %s) failed",
			handle, responseFile)
	}
...
// OnRequest provides the Go interface for E_PlainOtBatch3SessionOnRequest().
func (session *SellerSession) OnRequest(requestFile, responseFile string) error {
	...
	ret := bool(C.E_PlainOtBatch3SessionOnRequest(
		handle, requestFileCStr, responseFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_PlainOtBatch3SessionOnRequest(%v, %s, %s) failed",
			handle, requestFile, responseFile)
	}
...
// OnReceipt provides the Go interface for E_PlainOtBatch3SessionOnReceipt()
func (session *SellerSession) OnReceipt(receiptFile, secretFile string) error {
	...
	ret := bool(C.E_PlainOtBatch3SessionOnReceipt(
		handle, receiptFileCStr, secretFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_PlainOtBatch3SessionOnReceipt(%v, %s, %s) failed",
...
// NewBuyerSession provides the Go interface for E_PlainOtBatch3ClientNew()
func NewBuyerSession(
	...
	if session == nil {
		b.Free()
		return nil, fmt.Errorf(
			"E_PlainOtBatch3ClientNew(%v, %v, %v, %v, %d, %v, %d) failed",
...
// Free provides the Go interface for E_PlainOtBatch3ClientFree()
func (session *BuyerSession) Free() error {
	handle := C.handle_t(session.handle)
	ret := bool(C.E_PlainOtBatch3ClientFree(handle))
	if !ret {
		return fmt.Errorf("E_PlainOtBatch3ClientFree(%v) failed", handle)
	}
	return session.b.Free()
}

// GetNegoRequest provides the Go interface for E_PlainOtBatch3ClientGetNegoRequest()
func (session *BuyerSession) GetNegoRequest(requestFile string) error {
	...
	if ret := bool(
		C.E_PlainOtBatch3ClientGetNegoRequest(handle, requestFileCStr)); !ret {
		return fmt.Errorf("E_PlainOtBatch3ClientGetNegoRequest(%v, %s) failed",
			handle, requestFile)
	}
...
// OnNegoResponse provides the Go interface for E_PlainOtBatch3ClientOnNegoResponse()
func (session *BuyerSession) OnNegoResponse(responseFile string) error {
	...
	if ret := bool(C.E_PlainOtBatch3ClientOnNegoResponse(
		handle, responseFileCStr)); !ret {
		return fmt.Errorf(
			"E_PlainOtBatch3ClientOnNegoResponse(%v, %s) failed",
			handle, responseFile)
	}
...
// OnNegoRequest provides the Go interface for E_PlainOtBatch3ClientOnNegoRequest()
func (session *BuyerSession) OnNegoRequest(requestFile, responseFile string) error {
	...
	if ret := bool(C.E_PlainOtBatch3ClientOnNegoRequest(
		handle, requestFileCStr, responseFileCStr)); !ret {
		return fmt.Errorf(
			"E_PlainOtBatch3ClientOnNegoRequest(%v, %s, %s) failed",
			handle, requestFile, responseFile)
	}
...
// GetRequest provides the Go interface for E_PlainOtBatch3ClientGetRequest()
func (session *BuyerSession) GetRequest(requestFile string) error {
	...
	if ret := bool(
		C.E_PlainOtBatch3ClientGetRequest(handle, requestFileCStr)); !ret {
		return fmt.Errorf("E_PlainOtBatch3ClientGetRequest(%v, %s) failed",
			handle, requestFile)
	}
...
// OnResponse provides the Go interface for E_PlainOtBatch3ClientOnResponse()
func (session *BuyerSession) OnResponse(responseFile, receiptFile string) error {
	...
	ret := bool(C.E_PlainOtBatch3ClientOnResponse(
		handle, responseFileCStr, receiptFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_PlainOtBatch3ClientOnResponse(%v, %s, %s) failed",
...
// OnSecret provides the Go interface for E_PlainOtBatch3ClientOnSecret()
func (session *BuyerSession) OnSecret(secretFile string) error {
	...
	ret := bool(C.E_PlainOtBatch3ClientOnSecret(handle, secretFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_PlainOtBatch3ClientOnSecret(%v, %s) failed",
			handle, secretFile)
	}
...
// Decrypt provides the Go interface for E_PlainOtBatch3ClientSaveDecrypted()
func (session *BuyerSession) Decrypt(outFile string) error {
	...
	ret := bool(C.E_PlainOtBatch3ClientSaveDecrypted(
		handle, outFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_PlainOtBatch3ClientSaveDecrypted(%v, %s) failed",
			handle, outFile)
	}
