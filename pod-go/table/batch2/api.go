...
// NewSellerSession provides the Go interface for E_TableBatch2SessionNew().
func NewSellerSession(
	...
	if session == nil {
		...
		return nil, fmt.Errorf(
			"E_TableBatch2SessionNew(%v, %v, %v) failed",
...
// Free provides the Go interface for E_TableBatch2SessionFree()
func (session *SellerSession) Free() error {
	handle := C.handle_t(session.handle)
	ret := bool(C.E_TableBatch2SessionFree(handle))
	if !ret {
		return fmt.Errorf("E_TableBatch2SessionFree(%v) failed", handle)
	}
	return session.a.Free()
}

// OnRequest provides the Go interface for E_TableBatch2SessionOnRequest().
func (session *SellerSession) OnRequest(requestFile, responseFile string) error {
	...
	ret := bool(C.E_TableBatch2SessionOnRequest(
		handle, requestFileCStr, responseFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_TableBatch2SessionOnRequest(%v, %s, %s) failed",
...
// OnReceipt provides the Go interface for E_TableBatch2SessionOnReceipt()
func (session *SellerSession) OnReceipt(receiptFile, secretFile string) error {
	...
	ret := bool(C.E_TableBatch2SessionOnReceipt(
		handle, receiptFileCStr, secretFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_TableBatch2SessionOnReceipt(%v, %s, %s) failed",
...
// NewBuyerSession provides the Go interface for E_TableBatch2ClientNew()
func NewBuyerSession(
	...
	if session == nil {
		b.Free()
		return nil, fmt.Errorf(
			"E_TableBatch2ClientNew(%v, %v, %v, %v, %d) failed",
...
// Free provides the Go interface for E_TableBatch2ClientFree()
func (session *BuyerSession) Free() error {
	handle := C.handle_t(session.handle)
	ret := bool(C.E_TableBatch2ClientFree(handle))
	if !ret {
		return fmt.Errorf("E_TableBatch2ClientFree(%v) failed", handle)
	}
	return session.b.Free()
}

// GetRequest provides the Go interface for E_TableBatch2ClientGetRequest()
func (session *BuyerSession) GetRequest(requestFile string) error {
	...
	if ret := bool(
		C.E_TableBatch2ClientGetRequest(handle, requestFileCStr)); !ret {
		return fmt.Errorf("E_TableBatch2ClientGetRequest(%v, %s) failed",
			handle, requestFile)
	}
...
// OnResponse provides the Go interface for E_TableBatch2ClientOnResponse()
func (session *BuyerSession) OnResponse(responseFile, receiptFile string) error {
	...
	ret := bool(C.E_TableBatch2ClientOnResponse(
		handle, responseFileCStr, receiptFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_TableBatch2ClientOnResponse(%v, %s, %s) failed",
...
// OnSecret provides the Go interface for E_TableBatch2ClientOnSecret()
func (session *BuyerSession) OnSecret(secretFile string) error {
	...
	ret := bool(C.E_TableBatch2ClientOnSecret(handle, secretFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_TableBatch2ClientOnSecret(%v, %s) failed",
			handle, secretFile)
	}
...
// Decrypt provides the Go interface for E_TableBatch2ClientSaveDecrypted()
func (session *BuyerSession) Decrypt(outFile string) error {
	...
	ret := bool(C.E_TableBatch2ClientSaveDecrypted(
		handle, outFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_TableBatch2ClientSaveDecrypted(%v, %s) failed",
			handle, outFile)
	}
