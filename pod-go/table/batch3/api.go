...
// NewSellerSession provides the Go interface for E_TableBatch3SessionNew().
func NewSellerSession(
	...
	if session == nil {
		...
		return nil, fmt.Errorf(
			"E_TableBatch3SessionNew(%v, %v, %v) failed",
...
// Free provides the Go interface for E_TableBatch3SessionFree()
func (session *SellerSession) Free() error {
	handle := C.handle_t(session.handle)
	ret := bool(C.E_TableBatch3SessionFree(handle))
	if !ret {
		return fmt.Errorf("E_TableBatch3SessionFree(%v) failed", handle)
	}
	return session.a.Free()
}

// OnRequest provides the Go interface for E_TableBatch3SessionOnRequest().
func (session *SellerSession) OnRequest(requestFile, responseFile string) error {
	...
	ret := bool(C.E_TableBatch3SessionOnRequest(
		handle, requestFileCStr, responseFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_TableBatch3SessionOnRequest(%v, %s, %s) failed",
...
// OnReceipt provides the Go interface for E_TableBatch3SessionOnReceipt()
func (session *SellerSession) OnReceipt(receiptFile, secretFile string) error {
	...
	ret := bool(C.E_TableBatch3SessionOnReceipt(
		handle, receiptFileCStr, secretFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_TableBatch3SessionOnReceipt(%v, %s, %s) failed",
...
// NewBuyerSession provides the Go interface for E_TableBatch3ClientNew()
func NewBuyerSession(
	...
	if session == nil {
		b.Free()
		return nil, fmt.Errorf(
			"E_TableBatch3ClientNew(%v, %v, %v, %v, %d) failed",
...
// Free provides the Go interface for E_TableBatch3ClientFree()
func (session *BuyerSession) Free() error {
	handle := C.handle_t(session.handle)
	ret := bool(C.E_TableBatch3ClientFree(handle))
	if !ret {
		return fmt.Errorf("E_TableBatch3ClientFree(%v) failed", handle)
	}
	return session.b.Free()
}

// GetRequest provides the Go interface for E_TableBatch3ClientGetRequest()
func (session *BuyerSession) GetRequest(requestFile string) error {
	...
	if ret := bool(
		C.E_TableBatch3ClientGetRequest(handle, requestFileCStr)); !ret {
		return fmt.Errorf("E_TableBatch3ClientGetRequest(%v, %s) failed",
			handle, requestFile)
	}
...
// OnResponse provides the Go interface for E_TableBatch3ClientOnResponse()
func (session *BuyerSession) OnResponse(responseFile, receiptFile string) error {
	...
	ret := bool(C.E_TableBatch3ClientOnResponse(
		handle, responseFileCStr, receiptFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_TableBatch3ClientOnResponse(%v, %s, %s) failed",
...
// OnSecret provides the Go interface for E_TableBatch3ClientOnSecret()
func (session *BuyerSession) OnSecret(secretFile string) error {
	...
	ret := bool(C.E_TableBatch3ClientOnSecret(handle, secretFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_TableBatch3ClientOnSecret(%v, %s) failed",
			handle, secretFile)
	}
...
// Decrypt provides the Go interface for E_TableBatch3ClientSaveDecrypted()
func (session *BuyerSession) Decrypt(outFile string) error {
	...
	ret := bool(C.E_TableBatch3ClientSaveDecrypted(
		handle, outFileCStr))
	if !ret {
		return fmt.Errorf(
			"E_TableBatch3ClientSaveDecrypted(%v, %s) failed",
			handle, outFile)
	}
